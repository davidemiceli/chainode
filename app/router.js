...
const verify_sign = require('./middlewares/verify_sign');
const is_newpeer = function(req, res, next) {
  req.newpeer = true;
  return next();
}
...
router.post(Routes.PERMISSION.JOIN.ASK, is_auth, whoiam, peer_required, sign, require('./routes/permission/join/ask'));
router.post(Routes.PERMISSION.JOIN.RECEIVE, whoiam, peer_required, is_blockgenerator, is_newpeer, verify_sign, require('./routes/permission/join/receive'));
...
// Transactions
router.post(Routes.TRANSACTION.PROPOSE, whoiam, peer_required, join_required, sign, require('./routes/transaction/propose'));
router.post(Routes.TRANSACTION.RECEIVE, whoiam, peer_required, is_blockgenerator, verify_sign, require('./routes/transaction/receive'));
// Blocks methods
router.get(Routes.BLOCKS.SYNC, whoiam, peer_required, join_required, require('./routes/blocks/sync'));
router.get(Routes.BLOCKS.LATEST, whoiam, peer_required, require('./routes/blocks/latest'));
router.post(Routes.BLOCKS.LATEST, whoiam, peer_required, require('./routes/blocks/latest'));
